...
void WriteCsvHeader()
{
    ...
    fprintf(fp, "Src1MemType,");
    fprintf(fp, "Src2MemType,");
    fprintf(fp, "Dst1MemType,");
    fprintf(fp, "Dst2MemType,");
                ...
                int const dst2MemType)
{
    ...
    HIP_CALL(hipEventCreateWithFlags(&eventStart, hipEventReleaseToSystem));
    HIP_CALL(hipEventCreateWithFlags(&eventStop, hipEventReleaseToSystem));
    hipStream_t stream;
    ...
    for (int iter = -config.numWarmup; iter < config.numIterations; iter++)
    {
        ...
        testFunc(config, gpuMem, numWorkgroups, blockSize,
                 src1MemType, src2MemType,
                 dst1MemType, dst2MemType, stream);
    ...
    validateFunc(gpuMem, src1MemType, src2MemType, dst1MemType, dst2MemType);

    printf("%-20s [%s|%s]->[%s|%s]: Min: %7.3f ms Avg: %7.3f ms Max: %7.3f ms (avg %7.3f GB/s)\n",
           name.c_str(),
           memTypeNames[src1MemType],
           memTypeNames[src2MemType],
           memTypeNames[dst1MemType],
           memTypeNames[dst2MemType],
           minTime, avgTime, maxTime, avgBandwidth);

    if (config.outputToCsv)
    {
        fprintf(fp, "%s,", name.c_str());
        fprintf(fp, "%s,", memTypeNames[src1MemType]);
        fprintf(fp, "%s,", memTypeNames[src2MemType]);
        fprintf(fp, "%s,", memTypeNames[dst1MemType]);
        fprintf(fp, "%s,", memTypeNames[dst2MemType]);
